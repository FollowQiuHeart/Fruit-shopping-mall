...
function check_login(){
    //向后台发异步请求
    $.get(url='/index/check_login',callback = function (data) {
        var html = ''
        //{loginState:0}
        if (data.login_state == 0 ){
            //未登录
            html = "<a href='/index/login'>[登录]</a>";
            html += "<a href='/index/register'>[注册,有惊喜]";

        }else{
            //{'loginState':1,'username':'guo...'}
            html += "欢迎:" + data.uname+" ";
...
function loadGoods(){
    $.get("/index/load_goods",function(data){
        //[{'type':{'title':,,}},"goods":[{'title':xx,}],}...]
        var show = ""
        $.each(data,function(i,obj){
            show += '<div class="title" id="main">'
				...
				show += '<li class="fl">'
					show += '<div class="box">'
						...
						show += '<div class="tip">'
							...
							show += '<div class="fr">'
								show += '<figure>'
									show += '<img src="/static/imgs/cart.png" alt="">'
								show += '</figure>'
							show += '</div>'
							show += '<div class="cb"></div>'
						show += '</div>'
